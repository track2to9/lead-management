...
여러 PDF 동시 드래그앤드롭 지원. 클라이언트가 파일을 **하나씩 순차로** `POST /api/quotations/import` 에 업로드하면서 진행 상태를 표시:
...
- 모달을 닫거나 페이지를 떠나면 남은 파일은 중단됨. 사용자가 다시 들어와 업로드하면 이어서 가능.
- 완료 시 토스트: `23개 임포트 완료, 검증 필요 5건`.
- 동시 병렬 업로드는 하지 않음 (서버 부하와 Anthropic rate limit 보호). 한 파일이 완전히 처리된 뒤 다음 파일 시작.
...
백그라운드 job 큐 / 워커 인프라를 만들지 않는다. 대신 **파일당 하나의 요청**으로 동기 처리한다. 이유:
- Next.js 16 Route Handler 는 serverless 환경에서 응답 후 백그라운드 작업을 계속하기 어렵다.
- 파일 한 건의 처리 시간(텍스트 추출 + LLM 호출)은 대략 5~30초로 serverless 타임아웃(60~300초) 안에 들어온다.
- 클라이언트가 파일 리스트를 순차 루프하면서 한 파일씩 POST 하면, 진행률은 자연스럽게 클라이언트가 추적한다. 별도 polling 엔드포인트 불필요.

### `POST /api/quotations/import`

- Content-Type: `multipart/form-data`
- Body: 단일 PDF 파일 (`file` 필드)
- 처리 (모두 요청 안에서 동기 실행):
  1. 파일을 Supabase Storage `quotation-imports/{user_id}/{new_uuid}/{filename}` 에 저장
  2. `unpdf` 로 단계 A 추출 (텍스트 + 표)
  3. Claude Haiku 4.5 로 단계 B 구조화 추출
  4. `quotations` + `quotation_items` row 생성 (`status='imported_unverified'`, `source='imported_pdf'`)
  5. 응답: 생성된 `Quotation` 전체 (items 포함) 또는 에러 상세
- 텍스트 추출 실패/LLM 실패 시에도 HTTP 200 으로 응답하되, `quotations` 행은 빈 쉘로 생성하고 `import_confidence` 에 사유 기록.
- 심각한 오류(파일 없음, 권한 없음, 등)는 HTTP 4xx/5xx.

클라이언트(ImportDropzone) 는 여러 파일을 선택해도 한 번에 한 파일씩 순차 POST 하며 진행 상태를 UI 에 반영한다.
...
- [ ] 업로드 중 모달에서 파일별 진행 상태(대기/처리중/완료/실패)를 확인할 수 있다.
